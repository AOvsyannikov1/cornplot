...
class PlotWindow(QWidget):
    ...
    def add_plot(self, row, col, x_arr, y_arr, name='', linewidth=2, linestyle='solid', color='any'):
        index = self.dashboard_locations.index((row, col))
        self.dashboards[index].add_plot(x_arr, y_arr, name=name, linewidth=linewidth,
    ...
    def add_histogram(self, row, col, data, intervals_count=0, name="", color="any"):
        index = self.dashboard_locations.index((row, col))
        self.dashboards[index].add_histogram(data, name=name, color=color, interval_count=intervals_count)
...
class CornPlotter:
    ...
    def plot(self, x_arr, y_arr, x_name="X", y_name="Y", name='',
        ...
        if len(self.__windows) == 0:
            self.__windows.append(PlotWindow())
            self.__windows[-1].setWindowTitle(f"Окно {len(self.__windows)}")
        self.__windows[self.__current_window_index].add_axes(self.__current_row, self.__current_col, rows=1, cols=1,
    ...
    def animated_plot(self, name: str, x_size=30, x_name="X", y_name="Y", linewidth=2, linestyle='solid', color='any', link_plots=True, axes=False):
        ...
        self.__windows[self.__current_window_index].dashboards[-1].set_x_name(x_name)
        self.__windows[self.__current_window_index].dashboards[-1].set_y_name(y_name)
        self.__windows[self.__current_window_index].add_animated_plot(self.__current_row, self.__current_col,
